...
#sleep 1
#cl run :src :lib :evaluator :atis :atis-lexicon "OMP_NUM_THREADS=4 THEANO_FLAGS=blas.ldflags=-lopenblas,device=gpu1,floatX=float32,cuda.root=/usr/local/cuda-7.0 python src/py/main.py -d $d -i $i -o $i -p none -u 1 -t 25,5,5 -c lstm -m attention -k 10 --stats-file stats.json --train-data atis-lexicon/atis_train_augmentLex.tsv --dev-data atis/atis_test.tsv --save-file params" --request-queue 'host=jagupard7' --request-cpus 1 -n atis_test -d 'augment lex' --request-docker-image codalab/theano-cuda7.0-352.39 
#sleep 1
