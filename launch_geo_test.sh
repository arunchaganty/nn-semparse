...
cl run :src :lib :evaluator :geo880-lesscopy :geo880-augmented "OMP_NUM_THREADS=4 THEANO_FLAGS=blas.ldflags=-lopenblas,device=${device},floatX=float32 python src/py/main.py -d $d -i $i -o $i -p none -u 1 -t 40,5,5 -c lstm -m encoderdecoder -k 10 --stats-file stats.json --domain geoquery --train-data geo880-lesscopy/geo880_train600.tsv --dev-data geo880-lesscopy/geo880_test280.tsv --save-file params" --request-queue jag --request-cpus $num_cpu -n geoquery_test -d '600, no augmentation' $flags
sleep 1
cl run :src :lib :evaluator :geo880-lesscopy :geo880-augmented "OMP_NUM_THREADS=4 THEANO_FLAGS=blas.ldflags=-lopenblas,device=${device},floatX=float32 python src/py/main.py -d $d -i $i -o $i -p none -u 1 -t 40,5,5 -c lstm -m encoderdecoder -k 10 --stats-file stats.json --domain geoquery --train-data geo880-augmented/geo880_train600_augpcfg300.tsv --dev-data geo880-lesscopy/geo880_test280.tsv --save-file params" --request-queue jag --request-cpus $num_cpu -n geoquery_test -d '600, augment 300' $flags
#sleep 1
