...
Second, semantic parsers understand compositionality:
...
\begin{itemize}
  ...
  paired with corresponding database queries written in a Prolog-like
  query language.
...
For example, place names in \geo and \atis can be copied,
...
\end{figure}
To perform compositional data augmentation on \geo,
we begin by defining a few high-precision alignment rules.
...
we define a grammar that can replace individual entity mentions with
...
Note that this procedure generates examples that are on average
...
Note that unlike our synthetic examples for \geo, 
