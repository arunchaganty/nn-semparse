...
This complexity motivates the use of 
models like recurrent neural networks (RNNs),
which require less engineering effort and 
...
together constitute a practical system for 
...
Existing semantic parsers have built-in prior knowledge
to handle both of these challenges, but a more generic
RNN architecture might be unable to generalize properly due to these challenges.
...
\rj{Here-ish?  Need to transition better}
...
\begin{enumerate}
  ...
  \item \textbf{Decoder Module}.  This module
    ...
    %stopping when it generates a special end-of-sentence token. % PL: standard/obvious detail
...
\begin{enumerate}
  ...
  \item \textbf{Decoder Output Module}.  This module
...
\rj{Sure, but then the backward recurrence has to start at time $n+1$ and go down to $1$}
...
This observation motivates our novel attention-based copying output module.
...
%(e.g. SEMPRE), which maximize log-likelihood of the correct
%denotation.
