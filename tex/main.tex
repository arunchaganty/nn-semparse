...
The closest related work we are aware of is
...
plus $NUMBER$ examples generated by the integer-based scheme
and $NUMBER$ randomly sampled new examples generated by the string-based scheme.
...
However, we lag significantly behind on \geo and \atis.
...
on \regex, by a considerable margin.  We are also more competitive
with state-of-art on \geo.  Our \atis performance still lags further behind.
...
When training on $n$ examples, we added $\frac{n}2$ synthetic examples
...
``are neighbors,'' etc.  All the synthetic examples
we generate involve the composition of relations to some depth,
plus a single entity.  For example, ``neighbors of friends of alice''
would be a depth-$2$ example.

We train our model to be able to understand depth-$2$ examples.
The model is given small training set of $100$ such examples.
We try augmenting this dataset in three ways:
\begin{enumerate}
  \item \textbf{Entity-based}.  Swap out entity mentions for different entity mentions.
    Our strategy on \regex was most similar to this strategy.
  \item \textbf{Nesting-based}.  Swap out one entity mention for another complete example,
    thereby nesting two examples and creating a depth-$4$ example.
  \item \textbf{Both}.  Apply (2) followed by (1).
    Our strategy on \geo was most similar to this strategy.
\end{enumerate}
...
and hence are \emph{out-of-domain} augmentation strategies.

In Figure \ref{fig:artificial}, we plot the accuracy on a test set 
of depth-$2$ examples versus the number of synthetic examples added
using each of the three augmentation strategies.
While all three augmentation strategies help,
the entity-based augmentation strategy
proves to be the least helpful of the three.
It is the only one that never achieves accuracy of $99\%$ or higher,
maxing out at $92.8\%$.
The best strategy is the combined one, which thrice achieves $100\%$ accuracy
on the test set.
Overall, we see that, at least in some cases,
\emph{out-of-domain} data augmentation can be very beneficial
to learning.
